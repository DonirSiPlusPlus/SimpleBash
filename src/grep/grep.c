...
grep_string pars_string = { .opt_i = 0, .opt_v = 0, .opt_c = 0, .opt_l = 0, .opt_n = 0,
.opt_h = 0, .opt_s = 0, .opt_f = 0, .opt_o = 0,
.count_keys = 0, .count_files = 0,
.keywords = NULL, .file_args = NULL };
        ...
        if (argv[i][0] == '-') {
            switch (argv[i][1]) {
                case 'e':
                    ...
                    pars_string.opt_e++;
                    break;
                case 'i': pars_string.opt_i = 1; break;
                case 'v': pars_string.opt_v = 1; break;
                case 'c': pars_string.opt_c = 1; break;
                case 'l': pars_string.opt_l = 1; break;
                case 'n': pars_string.opt_n = 1; break;
                case 'h': pars_string.opt_h = 1; break;
                case 's': pars_string.opt_s = 1; break;
                case 'f':
                    if (i + 1 >= argc) {
                    ...
                    pars_string.opt_f = 1;
                    break;
                case 'o': pars_string.opt_o = 1; break;
                case '?': default:
        ...
        if ((my_file = fopen(pars_string.file_args[i], "r")) == NULL) {
            ...
            while (!feof(my_file)) {
                ...
                for (int j = 0; j < pars_string.count_keys; j++) {
                    regex_t preg;
                    if (!regcomp(&preg, pars_string.keywords[j], REG_EXTENDED|REG_NEWLINE)) {
                        regmatch_t pm;
                                    ...
                                    if (!regcomp(&preg, pars_string.keywords[j], REG_EXTENDED|REG_NEWLINE)) {
                                        while (!regexec(&preg, str, 1, &pm, 0)) {
                                            ...
                                            if (regcomp(&preg, pars_string.keywords[j],
                                                REG_EXTENDED|REG_NEWLINE)) break;
                                        }
                                    }
                                    regfree(&preg);
                                    was_break = 1;
...
int file_patterns(char **argv, int argc, int file) {
    ...
    if ((patterns = (fopen(argv[file], "r"))) == NULL) {
        if (!pars_string.opt_s) fprintf(stderr, "grep: %s: No such file or directory\n", argv[file]);
    } else {
        ...
        while (fgets(pattern, max_size, patterns)) {
            if (pars_string.count_keys >= argc) {
                ...
                pars_string.keywords[argc + counter++] = (char*)malloc(max_size * sizeof(char));
            }
